"""Business logic and data access for the admin module.

All file paths and index names are imported from admin_constants.
"""
...
from app.modules.admin.constants.admin_constants import (
    CHOICES_FILE as _CHOICES_FILE,
    CRITERIA_FILE as _CRITERIA_FILE,
    LOCAL_CAMPAIGNS_FILE as _LOCAL_CAMPAIGNS_FILE,
    OUTPUT_DIR,
    PRODUCTS_FILE as _PRODUCTS_FILE,
    SOURCES_FILE as _SOURCES_FILE,
    VERSE_INDEX as _VERSE_INDEX,
    TARGET_INDEX as _TARGET_INDEX,
)
from app.core.references import IdPrefixes, MarcheToProductMapping
...
# Imported from core/references.py — no magic strings here
_MARCHE_TO_PRODUCTS = MarcheToProductMapping.MAPPING
...
_NAME_KEYWORDS: List[tuple] = [
    ...
    (["FISCALITE LOCALE", "CFE", "CVAE", "TAXE FONCIERE", "IAE", "BINS", "PROPERTY TAX",
      "FISCAL", "FL EXO", "FL BINS", "TAX RECOVERY", "SALES TAX",
...
def list_campaigns() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Short timeout: if verse index is slow, fall back to local campaigns immediately
        res = es._es.search(
            index=_VERSE_INDEX,
            query={"match_all": {}},
            size=5000,
...
def _resolve_account_name(account_id: str) -> str:
    """Look up company name from the account index using the Salesforce ID."""
    if not account_id:
        return ""
    try:
        ...
        res = es._es.search(
            ...
            query={"ids": {"values": [account_id]}},
            size=1,
            source=["Name"],
            request_timeout=3,
        )
        hits = res.get("hits", {}).get("hits", [])
        return hits[0]["_source"].get("Name", "") if hits else ""
    except Exception:
        return ""


def _extract_account_id(camp_acc: str, camp_name: str, fallback: str) -> str:
    """Extract a valid Salesforce account ID (starts with 001, 15-18 chars) from CampAcc__c."""
    # Salesforce account IDs start with "001" and are 15 or 18 alphanumeric chars
    matches = re.findall(r'001[A-Za-z0-9]{12,15}', camp_acc)
    if matches:
        return matches[-1]          # last match is most likely the account suffix
    # Fallback: remove the known campaign name prefix
    stripped = camp_acc.replace(camp_name, "").strip()
    if re.match(r'^[A-Za-z0-9]{15,18}$', stripped):
        return stripped
    return fallback or ""
...
def get_campaign_targets(campaign_id: str, size: int = 50) -> Dict[str, Any]:
    ...
    try:
        ...
        for d in raw:
            ...
            account_id = _extract_account_id(camp_acc, camp_name, d.get("Compte__c", ""))
            account_name = (
                d.get("Name__c") or d.get("Account_Name__c")
                or _resolve_account_name(account_id)
                or account_id
            )
            if not account_id:
                continue   # skip targets without a resolvable Salesforce ID
            targets.append({
                "id":           d.get("Id"),

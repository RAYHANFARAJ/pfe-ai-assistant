...
class ESClientTool(AbstractElasticsearchClient):
    ...
    def search(self, query: str, size: int = 10) -> List[Dict[str, Any]]:
        ...
        Strategies tried (in order of relevance):
          1. Exact Salesforce ID match
          2. Prefix match on name  (e.g. "Soc" → "Société Générale")
          3. Fuzzy match on name   (e.g. "Societe" → "Société")
          4. Wildcard match        (e.g. "gene" → "Société GENErale")
        """
        q = query.strip()
            ...
            if not q
                ...
                "bool": {
                    "should": [
                        ...
                        {"ids":  {"values": [q]}},
                        {"term": {"Id": q}},
                        # Name prefix — best for autocomplete ("Soc" → "Société")
                        {"match_phrase_prefix": {"Name": {"query": q, "max_expansions": 20, "boost": 3}}},
                        # Fuzzy — handles typos and missing accents
                        {"match": {"Name": {"query": q, "fuzziness": "AUTO", "boost": 2}}},
                        # Wildcard — catches substrings anywhere in the name
                        {"wildcard": {"Name.keyword": {"value": f"*{q}*", "case_insensitive": True, "boost": 1}}},
                    ],
